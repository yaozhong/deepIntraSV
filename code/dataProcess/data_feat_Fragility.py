# functions used from Nature biotechnology paper.
...
# new added by Yao-zhong, to return the vector
def calc_fragility_vec(sequence, dinucleotides):

	vec = [0]*(len(sequence)-1)

	for i in range(len(sequence)):
		dint = str(sequence[i:i+2]).upper()
		if dint in dinucleotides:
			vec[i] = dinucleotides[dint]
	return vec

def calc_flexibility_vec(sequence):
	return calc_fragility_vec(sequence, flextab)

def calc_stability_vec(sequence):
	return calc_fragility_vec(sequence, helixtab)
