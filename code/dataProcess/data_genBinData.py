...
#######################################################################
def cache_trainData_NOTUSED(vcfFile, bamFile, dataPath, dataSplit):

    print("\n* [Bin Sample]: generating bin Sample data ... ")

    # this is the place catching the data. current is directly save the data
    if config.DATABASE["eval_mode"] == "single":
        cv_data, cnv_test_data = bkDataGen_singleSample(vcfFile, bamFile, config.DATABASE["data_aug"])
    # this part need to be carefully treated
    if config.DATABASE["eval_mode"] == "cross":
        cv_data, cnv_test_data = bkDataGen_crossSample(vcfFile, bamFile, config.DATABASE["data_aug"])
...
############################################
# 2019-11-21 , directly prepare data for the breakpoint_range
############################################
