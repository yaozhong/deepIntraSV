...
"""
# baisc U-net module
## not the conv length implemnation
"""
def UNet_module(rd_input, kernels, conv_window_len, maxpooling_len, stride=1, BN=True, DropoutRate=0.2):
            ...
            ##################### Conv2 ##########################
            conv2 = layers.Conv1D(kernels[1], conv_window_len, strides= stride, padding='same',\
            ...
            ##################### conv3 ###########################
            conv3 = layers.Conv1D(kernels[2], conv_window_len, strides= stride,  padding='same',\
            ...
            ####################  conv4 (U bottle) #####################
            conv4 = layers.Conv1D(kernels[3], conv_window_len, strides= 1, padding='same',\
            ...
            up5 = Conv1DTranspose(drop4, kernels[2] , conv_window_len, strides=maxpooling_len[2], padding='same')

            merge5 = layers.Concatenate(-1)([drop3, up5])

            conv5 = layers.Conv1D(kernels[2], conv_window_len, padding='same', \
            ...
            up6 = Conv1DTranspose(conv5, kernels[1] , conv_window_len, strides=maxpooling_len[1], padding='same')
            
            merge6 = layers.Concatenate(-1)([conv2, up6])
        
            conv6 = layers.Conv1D(kernels[1], conv_window_len, padding='same', \
            ...
            up7 = Conv1DTranspose(conv6, kernels[0] , conv_window_len, strides=maxpooling_len[0], padding='same')
            
            merge7 = layers.Concatenate(-1)([conv1, up7])

            conv7 = layers.Conv1D(kernels[0], conv_window_len, padding='same',\
            ...
            ################## final output ###################### 
            conv8 = layers.Conv1D(kernels[0], conv_window_len, padding='same', \
                kernel_initializer=initializer)(conv7)
...
# 2019-07-11 test module II, to test originl implemntaiton of U-net (relu-BN implemnation)
def UNet_module2(rd_input, kernels, conv_window_len, maxpooling_len, stride=1, BN=True, DropoutRate=0.2):

            initializer = 'he_normal' # 'glorot_uniform'

            ##################### Conv1 #########################      
            conv1 = layers.Conv1D(kernels[0], conv_window_len, activation='relu', strides= stride , padding='same', \
                kernel_initializer=initializer)(rd_input)
            if BN: conv1 = layers.BatchNormalization()(conv1)
            ...
            ##################### Conv2 ##########################
            conv2 = layers.Conv1D(kernels[1], conv_window_len, strides= stride, activation='relu',  padding='same',\
                kernel_initializer=initializer)(pool1)
            if BN: conv2 = layers.BatchNormalization()(conv2) 
            ...
            ##################### conv3 ###########################
            conv3 = layers.Conv1D(kernels[2], conv_window_len, strides= stride,  activation='relu', padding='same',\
                kernel_initializer=initializer)(pool2)
            if BN: conv3 = layers.BatchNormalization()(conv3) 
            ...
            conv5 = layers.Conv1D(kernels[2], conv_window_len, activation='relu', padding='same', \
                kernel_initializer=initializer)(merge5)
            if BN: conv5 = layers.BatchNormalization()(conv5) 
            ...
            conv6 = layers.Conv1D(kernels[1], conv_window_len, activation='relu', padding='same', \
                kernel_initializer=initializer)(merge6)
            if BN: conv6 = layers.BatchNormalization()(conv6) 
            ...
            conv7 = layers.Conv1D(kernels[0], conv_window_len, activation='relu',padding='same',\
                kernel_initializer=initializer)(merge7)
            if BN: conv7 = layers.BatchNormalization()(conv7) 
            ...
            ################## final output ###################### 
            conv8 = layers.Conv1D(kernels[0], conv_window_len, activation='relu',padding='same', \
                kernel_initializer=initializer)(conv7)
            if BN: conv8 = layers.BatchNormalization()(conv8) 
...
def UNet_only2(rd_input, kernels, conv_window_len, maxpooling_len,stride, BN=True, DropoutRate=0.2):

            unet_module_output = UNet_module2(rd_input, kernels, conv_window_len, maxpooling_len, stride, BN, DropoutRate)
            
            conv9 = layers.Conv1D(1, 1, activation='sigmoid')(unet_module_output)
...
def UNet_GRU3(rd_input, kernels, conv_window_len, maxpooling_len, stride, BN=True, DropoutRate=0.2):
            ...
            output = layers.Bidirectional(layers.CuDNNGRU(32, return_sequences=True))(output)
            output = layers.TimeDistributed(layers.Dense(2, activation="softmax"))(output)
...
def UNet_GRU2(rd_input, kernels, conv_window_len, maxpooling_len, stride, BN=True, DropoutRate=0.2):
            ...
            output = layers.Bidirectional(layers.CuDNNGRU(32, return_sequences=True))(unet_module_output)
            output = layers.Bidirectional(layers.CuDNNGRU(32, return_sequences=True))(output)
            output = layers.TimeDistributed(layers.Dense(1, activation="sigmoid"))(output)
...
def URNet_noEndRNN(rd_input, kernels, conv_window_len, maxpooling_len, stride, BN=True, DropoutRate=0.2):
            ...
            output = layers.Conv1D(1, 1, activation='sigmoid')(conv8)    
            #conv9 = layers.Conv1D(8, 1, activation='softmax')(conv8)
            model = models.Model(rd_input, output)

            return model

def URNet_1EndRNN(rd_input, kernels, conv_window_len, maxpooling_len, stride, BN=True, DropoutRate=0.2):
